...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df_filtered['class1']=df_filtered['class'].apply(lambda x: 'Successful' if (x==1) else 'Unsuccessful')
        df_count=df_filtered['class1'].value_counts().reset_index()
        data=pd.DataFrame(df_count)
        data.columns = ['class', 'counts']
        fig = px.pie(data, values='counts', 
        names='class', 
        title='Successful Launches vs Unsuccessful Launches from '+entered_site)
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site,payload_range):
    ...
    #df2=df1[df1['Payload Mass (kg)']>payload_range[0]]
    #df3=df2[df2['Payload Mass (kg)']<payload_range[1]]
